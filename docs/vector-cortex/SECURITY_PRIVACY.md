...
Security tests: permission creation/repair, encrypted backup round-trip/wrong-key failure, export/delete/retention, consent/revocation exclusion, and secret scanner over `conformance/vector-cortex/v2` plus assets/manifests. Runtime no-outbound-call enforcement is via the guardrails scanner's PREVENT-PI-004 rule (zero network calls at runtime **by default** — optional, user-configured features that require network are exempt when the user opts in): it statically scans TS/JS imports/calls for remote `fetch`/HTTP and audits the local dashboard (tailnet-local or loopback), loopback embedder, local Ollama HyDE/RAPTOR, and optional cost-API exceptions; there is no runtime network-patch test. Rust Cargo.lock/source and dashboard static scans also run; violations fail, not warn.
