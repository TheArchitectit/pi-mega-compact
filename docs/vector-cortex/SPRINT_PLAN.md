...
**ACTIVE SPRINT: VC0F — Dashboard Restart-on-Upgrade (session_start auto-restart).** VC0E shipped as v0.20.25 (honest status badges: LIVE / AWAITING DATA / DEFERRED instead of bare zeros), but exposed a durability gap: `pi update --extensions` swaps the on-disk package while long-running `_dashboard-runner.mjs` processes keep serving the old in-memory bundle. After v0.20.25, six orphan servers on :9320–9325 were still reporting v0.20.24. The immediate mitigation is in `scripts/deploy.sh` (post-publish bounce of local runners — commit `ea2e097`); VC0F is the durable, device-side complement: lift the existing stale-replace block out of the `/mega-dashboard` handler into a reusable `bounceStaleRunnerIfAny()` called from `session_start`, with a once-per-process gate and `port.pid` version stamping. See [sprint spec](sprints/VC0F-dashboard-restart-on-upgrade.md).
