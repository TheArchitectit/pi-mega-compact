...
**ACTIVE SPRINT: VC0E — Dashboard Live Data + Status Indicators.** All 27 sprints (VC0A→VC8C) + the VC0D production-wiring follow-up are complete at code level. VC0E fixes the wiring gaps the shipped dashboard exposes: emitters that are defined but never called (VC5B render seam, VC0A recall latency), wired-but-degenerate decisions (VC5C `decideLivePath` missing `clock`/`evidence`), routes that hardcode zeros without consulting `events.log` (rollout, outcomes, platform), and every card rendering zeros without showing status. See [sprint spec](sprints/VC0E-dashboard-live-data.md). Branch: `feat/vc-dashboard-live-data`.
