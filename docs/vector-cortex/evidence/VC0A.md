...
- `node --test dist/src/vector-cortex/vc0a-acceptance.test.js` → `tests 8, pass 8, fail 0` (flag ON); `MEGACOMPACT_VC0A=0 node --test ...` → `tests 8, pass 8, fail 0` (flag-off rehearsal; mode C zero writes).
- `src/vector-cortex/eval/*.test.js` (metrics 7, annotations 6, persist 4) + acceptance → all pass.
- `npm test` → `TOTAL: 1252 passed, 0 failed across 174 files in 23.1s`.
...
- `MEGACOMPACT_VC0A` surfaced in `SETTINGS` (new "Vector Cortex" group).
