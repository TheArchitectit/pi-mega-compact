...
- **VC1A-I09 [type: minor, state: fixed-in-this-sprint]**: the I05 regression assertions verified the returned `ValidationResult.issues[]` locators but did NOT assert the actually-EMITTED `validation_failed` event's fields — the original defect was specifically empty locator FIELDS on the emitted event. Fixed: the acceptance test's emitter now captures `(event, fields)` and asserts the emitted `session`/`eventId`/`seq`(stringified)/`code` fields directly (in addition to the returned `issues[]`).
