...
Implementation commits/sub-sprint gates: `feat(COS-FP-R): real-corpus L2 cosine FP validation harness + flag` (a755132) + `docs(COS-FP-R): record no_corpus execution evidence` (bdee87d) + `docs(COS-FP-R): pin evidence commit SHAs` (this fixup) on `feat/COS-FP-R`; full gate run on the working tree (build / test / lint / regression / guardrails / conformance / docs-check / scope-check / evidence-check).
