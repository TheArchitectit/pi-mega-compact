...
- `src/vector-cortex/ledger/store.ts` (427) — `createLedgerStore(dbPath, emit?)`: byte-authority `sha256:` digesting over raw `sourceBytes`, capability-gated `reader()/writer()/admin()`, journal auto-record on append, admin `migrateOccurrenceV2()` + `compat` surface, and the emit seam. The writer primitive is intentionally NOT flag-gated — the flag gates only the ingestion seam + emit seam, so the writer stays fully functional under flag OFF (M2/tool-identity acceptance runs through the real writer regardless of flag; a primitive-level no-op proved over-broad and was reverted). `legacyProjectionOf` returns `null` for non-UTF-8 bytes (`TextDecoder("utf-8",{fatal:true})` in try/catch) — makes `MIG-DOWN-003` reachable through the real writer.
...
- `src/vector-cortex/vc1b-acceptance.test.ts` (600) — 25-test acceptance aggregator: conformance registration (M2-001..015, MIG-DOWN-001, named behaviors, EVT-016..030), M2 lifecycle, MIG-DOWN/named fixtures, capability gating, append-only provenance (no `UPDATE` in schema), flag-gated emit seam, **flag-off emission parity** (S2: flag OFF emits zero events while the writer primitive stays fully functional — both flag states run the same 25 tests through the real store).
...
- Acceptance, mandated command, both flag states:
  ...
  # → ℹ tests 25, ℹ pass 25, ℹ fail 0   (both flag states run the full 25 tests green)
...
`MEGACOMPACT_VC1B=0` → the ingestion SEAM opens NO ledger DB: `openLedgerWriter`/`appendMessagesToLedger` return 0 / null and never create `occurrence-v2.db` (byte-identical predecessor on the live path); the store writer primitive stays fully functional and the emit seam emits nothing. Acceptance passes in both flag states (0 failed each). Downgrade path (M2/MIG-DOWN) verified: prepare lists unrepresentable rows before building the legacy copy; the new legacy copy is produced on switch with unrepresentable rows listed; an untouched ledger refuses the downtime path (`MIG_DOWN_NOT_ACTIVE`). Evidence retained on rollback.
...
- **VC1B-I04 [type: spec-compliance, state: fixed-in-this-sprint]**: independent review flagged S1 (spec said "wire only writer capability to ingestion" but nothing called `store.writer()`) and S2 (the flag gated only the emit seam, so the live write path was not gated). Fixed: S1 wires the writer into the pi runtime ingestion path via the new `extensions/mega-runtime/vector-cortex-ledger.ts` adapter (opens NO ledger DB when flag OFF), called from `context-handler.ts`; S2 gates that ingestion SEAM. New adapter test (`vector-cortex-ledger.test.ts`, 4 tests) covers the seam gate.
- **VC1B-I05 [type: regression, state: fixed-in-this-sprint]**: the first S2 fix wrongly gated the store PRIMITIVE — `writer().append()` returned a `syntheticAppend` no-op under flag OFF — which broke 12/25 flag-off acceptance tests (M2-001/002/010-015, MIG-DOWN-001, M2-DUP-001, M2-TOOL-002, MIG-DOWN-003) because the M2 lifecycle + tool-identity suite exercises the real store regardless of flag. Reverted the primitive gate entirely (`syntheticAppend` removed), keeping flag gating solely at the ingestion seam + emit seam; the flag-off acceptance test now asserts emission-only parity (primitive stays fully functional). `MEGACOMPACT_VC1B=0` acceptance re-verified 25/25.
- **VC1B-Q01..Q06 [type: code-quality, state: fixed-in-this-sprint]**: independent code-quality review. Q01 — a dedup-accept returned the input's occurrence (caller seq), so `compat.record` wrote a phantom journal row at a seq with no backing occurrence row → false `SEQ_REGRESSION` on downgrade. Fixed: the dedup path now returns the EXISTING stored occurrence (its real seq), so the journal row always corresponds to a real occurrence_v2 row. Q02 — the ingestion seam swallowed append failures silently; now `appendMessagesToLedger` accepts an `onFailure` logger and the context-handler wires `runtime.logger` to emit `vc1b-ledger-append-skip` with the reason (e.g. `EVT_SEQ_REGRESSION` on rewind/fork), plus the monotonic-prefix assumption is documented. Q03 — `validate()`/`m2Validate()` only checked the `sha256:` prefix; they now RECOMPUTE sha256 over the stored base64 source and compare to the recorded digest via the shared `verifyLegacyDigest` (real DIGEST_MISMATCH parity check). Q04 — removed dead scaffolding: `base64Alphabet()`/`CHARS`, `freezeDerivedFrontier`, and `advanceHighWater` (interface + impl + the now-unused sqlite primitive); `isUnrepresentable` was retained (live consumers in the acceptance + journal tests verify MIG-DOWN-003/Q06 flagging). Q05 — replaced `v.codes as unknown as string[]` (store.ts) and `v.codes as M2MigrationCode[]` (occurrence-v2.ts) with real narrowing (`isM2MigrationCode` type guard). Q06 — an unrepresentable row no longer stores a base64 projection (`record()` writes NULL), and the dead `defaultLegacyProjection` fallback was removed. All 25 acceptance tests pass in both flag states; full `npm test` gate 0 failures.

## Residual risks / carried-forward OPEN issues

- **Carried forward OPEN (VC1 family):** the writer is now wired into the runtime ingestion path (S1: `context-handler.ts` → `appendMessagesToLedger`), but the toolCallId back-edge (the exactly-one tool-call→result identity) is NOT yet produced on the live path — the pi runtime discriminates `AgentMessage` by `role` only, and the tool-call id lives in variant content unreachable by role, so `messageToLedgerInput` omits toolCallId (documented deferral). `MEGACOMPACT_VC1B` gates the ingestion seam + emit seam. The full producer hook-up (appending per-turn occurrences with the exactly-one tool identity, including engine-level wiring beyond the context-handler seam) is deferred to the VC1 producer-wiring sprint.
