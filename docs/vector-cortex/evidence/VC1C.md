...
- `src/vector-cortex/conformance/manifest.ts` (282) — `FixtureManifestEntry`/`FixtureManifestV2`, `readFixtureManifestV2`, `validateCanonicalV2`, `canonicalManifestsConverge`, `domainOf`, `CONF_FAIL` (`CONF_EXTRA_FIXTURE`/`CONF_MISSING_FIXTURE`/`CONF_DIGEST_DRIFT`/`CONF_NONCANONICAL`/`CONF_UNKNOWN_DOMAIN`). A missing fixture reports `CONF_MISSING_FIXTURE` instead of throwing (robustness fix below).
- `src/vector-cortex/conformance/runner.ts` (105) — `ConformanceHandler`, `DowngradeReport` (schema `"downgrade-report-v1"`, `reportDigest`), `DowngradeExporter`, `runConformanceCase` (strict domain/version dispatch; unknown → reject WITHOUT partial output), `handlerKey`, `runDowngradeExport`.
...
- Acceptance, mandated command, both flag states:
  ...
  # → ℹ tests 27, ℹ pass 27, ℹ fail 0   (flag ON)
  MEGACOMPACT_VC1C=0 node --test dist/vector-cortex/vc1c-acceptance.test.js
  # → ℹ tests 27, ℹ pass 27, ℹ fail 0   (flag OFF: same 27 green — parity at the seam)
  ```
...
- `vc1c-acceptance.test.ts` (514) exceeds the `tests/` 300-line SOFT limit — consistent single-file acceptance aggregator, under the 600 HARD limit.
