...
- **Pre-existing failures (RESOLVED after this sprint shipped).** At the time VC5B shipped, `python3 scripts/log_failure.py --list` showed two `active` runtime failures carried since prior sprints (`FAIL-38192431` compaction "Already compacted", `FAIL-55d81817` S38 error-retry loop) — both in other sprints' domains (compaction / retry), not touched by or introduced by VC5B. Both were later verified shipped (FAIL-38192431 fix in commit `848c817`; FAIL-55d81817 R1/R2/R3 layers in `extensions/mega-events/error-classifier.ts`) and are now marked `resolved` in the failure ledger (gap-fill pass, post-VC5C).
