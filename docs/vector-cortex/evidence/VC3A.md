...
- `git diff --check` → exit 0 (no output). Verbatim: `git diff --check` prints nothing and returns 0 — the acceptance aggregator no longer ends in a trailing blank line (Q01).
...
## Code-quality resolution (Q01–Q04)

- **Q01 [gate]**: `git diff --check` failed (exit 2) with `new blank line at EOF` because `src/vector-cortex/vc3a-acceptance.test.ts` ended in a trailing blank line. Removed the trailing blank line so the file ends with a single `\n`; the gate now exits 0 with no output (verbatim evidence above).
- **Q02 [concurrency / idempotent-ack contract]**: `insertCortexRecord` did a check-then-insert with no enclosing transaction; a concurrent writer that lost a composite-PK race had its `UNIQUE constraint failed` error collapsed to `CTX_APPEND_FAILED` (a storage failure), breaking the idempotent-ack contract (CTX-004). Added an `idempotentOrConflict` helper shared by the pre-insert SELECT and the INSERT catch path: on any INSERT exception the committed key is re-checked and returned as an idempotent ack (same digest) or `CTX_KEY_CONFLICT` (different digest); only a genuine storage failure (e.g. `PRAGMA query_only` / SQLITE_FULL, where no row commits) still surfaces `CTX_APPEND_FAILED`. No type/contract change to `CortexAppendResult`.
- **Q03 [dead default producer]**: `createCortexStore` accepted an optional `emit` and only used `defaultEmitFor()` (the "REAL producer") inside `createCortexReporter`, so the default never ran in production and the comment overstated live behavior. Wired the default emitter into `createCortexStore` itself — a writer/admin host that invokes the seam without injecting an emitter now gets real structured-logger telemetry; an explicit `emit:` still replaces it. The dashboard reader-only route never appends/rebuilds, so it still fires nothing; a future VC3B host integration now has a working default observability seam.
- **Q04 [unreachable rebuild reuse]**: `rebuildCortexGeneration` documented idempotency but its reuse branch only looked up by the ordinal-derived `generationId` (always new in the no-opts flow), so every rebuild of unchanged inputs appended a fresh duplicate generation. Replaced it with a rootDigest+recordCount scan (`generationByRootDigest`): a rebuild whose sorted set is unchanged now reuses the identical existing generation (activates + returns it) instead of appending duplicate rows — one generation, not gen-1, gen-2, ... carrying the same digest. The doc comment was updated to describe the now-reachable reuse semantics.
