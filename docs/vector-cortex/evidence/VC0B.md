...
- **Triad mode B independence (informational, surfaced to maintainer):** `runReplayV2` has no separate A/B branch — both modes run `largestPairSafeSeq` + `computeEffectiveCutV2`, so the mode-B equality test compares code to itself. Resolving A/B independence is the controller's call; VC0C's real breaker+spool is where it gets exercised properly. Not fixed this round per review (F3).
