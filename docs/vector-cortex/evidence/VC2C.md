...
- `extensions/dashboard-client/src/types/vector-cortex.ts` — `encoderAssetDigest`/`encoderMode` on `VectorCortexHealthCard` (client typecheck+build green).
...
- `src/vector-cortex/encoder/select.test.ts` (new, ~171) — atomic eligibility (fully-satisfactory → A; every asset field enforced; per-head failures semantic/dependency/contradiction-ECE/cache/reconstruction demote the entire A), stable `qualificationManifestDigest`, corrupt-manifest injection → `ENC_QUALIFICATION_DIGEST_MISMATCH` choose B, matching digest does not demote.
...
- Acceptance, mandated command, both flag states:
  ...
  ```
- Unit: `calibrate.test.js` → 8 pass / 0 fail; `select.test.js` → 10 pass / 0 fail; `fallback.test.js` → 7 pass / 0 fail (25 total).
- `npm test` → `TOTAL: 1784 passed, 0 failed across 212 files in 27.0s`.
...
All 20 acceptance tests pass in both flag states (0 failed each). Encoder unit suites: `calibrate.test.js` (8) + `select.test.js` (10) + `fallback.test.js` (7). Invariants: calibration fit is invariant to row order (stable score/id ties, canonical split digest); a repo/session group never crosses a split boundary; held-out labels are strictly prohibited from the fit (`ENC_QUALIFICATION_HELD_OUT_IN_FIT`); eligibility is atomic across every MODEL_ASSET + per-head EVALUATION threshold; a single failed causal head demotes the entire A; the corrupt-qualification-manifest injection returns `ENC_QUALIFICATION_DIGEST_MISMATCH` and chooses B. Full `npm test` gate: `TOTAL: 1784 passed, 0 failed across 212 files`.
...
- The health card now surfaces `encoderAssetDigest` (SHA-256 of the committed qualified manifest) and `encoderMode` (A/B/C) as reader-only aggregates; the dashboard health-route test asserts both fields. Dashboard client `typecheck` + `build` both pass.
