...
- `vc6b-acceptance.test.ts` (391, **31 tests**) — acceptance aggregator over REAL `restoreSources` → `verifyRestored` (no mocks/stubs). Drives `HEAL-016..030` + the 3 named rows from the conformance corpus; asserts per-fixture mode/restoredCount/missingCount/codes; the byte-identity invariant (every restored span's bytes rebuilt independently from the decoded sources and compared verbatim — exact-shard payload, or the seq-sorted ledger concatenation); every insertion carries a request-VERIFIED digest; the unique failure injections; the forced A/B/C triad on one fixed span (proving B is byte-identical to A and C fabricates nothing); a 64-span boundary sweep with arbitrary non-UTF8 payloads plus a 70-span rejection; and flag-off byte-identical parity. Passes 31/31 under both flag states.
