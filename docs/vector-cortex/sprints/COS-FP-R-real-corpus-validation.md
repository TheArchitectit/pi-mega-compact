...
**Status: planned — execution blocked on corpus availability (not deferred; executes when corpus exists).** This spec is executable as specced. The corpus validation script is committed now; its execution requires only a consented corpus under `scripts/cosine-fp/corpus/`. The flag `MEGACOMPACT_COSINE_FP_REAL` is implemented now (default ON, `=0` byte-identical). `scripts/cosine-fp/real-bench.mjs` runs immediately when a valid corpus directory exists; the sprint's evidence doc records whether corpus-execution was possible or returned `no_corpus`. | **Depends on:** external-audit #3 (real half), COS-FP-A (synthetic baseline + harness pattern) | **Phase:** COS-FP
...
Cross-cutting seam: `extensions/dashboard-server/` files are auto-allowed by the scope gate even outside this ownership list; `docs/vector-cortex/evidence/COS-FP-R.md` (this sprint's evidence record) and `docs/vector-cortex/sprints/COS-FP-R-real-corpus-validation.md` (this spec) are likewise auto-allowed. **Production ownership:** `src/config/vector-cortex-cosfp-real.ts (new — MEGACOMPACT_COSINE_FP_REAL flag, sprintFlag sibling extract mirroring vector-cortex-cosfp.ts); src/config/vector-cortex.ts (additive re-export, stays ≤ 300); src/config.ts (additive re-export, ONE exported name, stays ≤ 220); scripts/cosine-fp/real-bench.mjs (new — corpus-gated real-corpus FP bench, imported helpers reused from ./bench.mjs, no duplication); scripts/cosine-fp/real-bench.test.mjs (new — write-time structural tests of the gates + Wilson/bootstrap/determinism); src/vector-cortex/cosfp-acceptance.test.ts (additive — COSINE_FP_REAL_ENABLED live-boolean assertion block); extensions/dashboard-server/routes-rag-settings-vector-cortex.ts (additive — visible COS-FP-R boolDirect SETTINGS toggle, seam)`
...
6. Emit `docs/vector-cortex/evidence/COS-FP-R.md` per [EVIDENCE_TEMPLATE](../EVIDENCE_TEMPLATE.md) at sprint execution: records the corpus-gate outcome (`no_corpus` if corpus absent, `corpus_invalid` if present but invalid, or the full run results if valid). The evidence doc is always written — `no_corpus` is a valid evidence record, not a deferred state.
...
## Corpus-absent behaviour (pre-corpus state)

**This sprint ships all code now**: the flag is implemented, the corpus validation script is committed, and the acceptance aggregator runs today. COS-FP-R owns **no conformance fixtures** (real corpus only; no synthetic substitute). `deploy.sh` runs as normal (flag default ON ships the code path; `=0` gates it off). The evidence doc `docs/vector-cortex/evidence/COS-FP-R.md` records the execution outcome: `no_corpus` if the corpus directory is absent, `corpus_invalid` if present but invalid, or the full run results if valid and consented. `no_corpus` evidence is a valid completed sprint state — it correctly records that real-corpus execution was attempted and found no consented data, which is the normative pre-donation state.

## Tests, validation, and assertions

COS-FP-R owns **no conformance fixtures** (real corpus only); the shared `cosfp-acceptance.test.ts` aggregator runs today. The execution script (`real-bench.mjs`, task 2) carries its own **write-time validation tests** (run when the corpus exists): (a) corpus-gate test — the script refuses to run on an empty/invalid manifest with status `no_corpus`/`corpus_invalid` and writes nothing; (b) consent-filter test — non-consented/revoked sessions are excluded from scoring and the denial is logged, never silently included; (c) CI test — Wilson interval bounds are in [0,1] and bootstrap(10000) is session-grouped (a session never straddles folds); (d) determinism test — same corpus + params → same result digest (mirrors COS-FP-A-002). The corpus-gate check runs at execution time (returns `no_corpus` when the corpus dir is absent — a valid completed sprint state, not a failure):
...
All standard gates run today (the code ships): `deploy.sh`, evidence check, scope-check, etc. `node scripts/vector-cortex-evidence-check.mjs COS-FP-R` expects an evidence doc — which exists and records `no_corpus` (valid completed state for the pre-corpus run). No gate is bypassed or papered over.
