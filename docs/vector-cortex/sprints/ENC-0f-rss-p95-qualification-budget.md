...
Production ownership: `scripts/encoder/gate-qualify.mjs (new — runs the real trained asset through the bench under --expose-gc, applies the 40ms-p95 + marginal-RSS + determinism + opset-21 gates, emits QualificationV1 + the two events); scripts/ml5/bench-onnx-prod.mjs (evolves — additive accept of a real trained asset path/threads, keep the existing WASM/native bench); src/vector-cortex/encoder/qualify.ts (new — pure QualificationV1 construction from the bench result: p95<=40, rss<=ENCODER_RSS_BUDGET_BYTES, deterministic, opset 21, all-gates); src/vector-cortex/encoder/qualify.test.ts (new — pure-fn unit tests); conformance/vector-cortex/v2/encoder-budget/ (fixtures ENC-BUDG-001..006); docs/vector-cortex/evidence/ENC-0f.md (new)`.
