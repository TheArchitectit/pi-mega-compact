...
Production ownership (as shipped): `training/vector-cortex/train_heads.py (new — five-head supervision transfer onto the frozen bge-small trunk, pure-stdlib deterministic-seeded: teacher-distill contradiction + NLI prior dependency + heuristic cache-stability + MLP payload-routing + CLS semantic; emits the head-candidate-v1 tree with per-head sha256 + trunkDigest pinning); training/vector-cortex/gen_synthetic_corpus.py (new — deterministic synthetic/self-labeled corpus + split grouped by repository+session, zero user bytes, 432 samples / 12 groups; writes the sibling dataset-manifest-enc0c.json, never the pre-existing ML5-A dataset-manifest.json); training/vector-cortex/train-v1.json (extended — frozenTrunk, trunk bge-small-en-v1.5, head keys cacheStability+payloadRouting, teacher config, loss weights, ENCODER_SEED 1729); training/vector-cortex/dataset-manifest-enc0c.json (new generated — every synthetic source with sha256/SPDX UNLICENSED-synthetic, split-group map, corpusDigest); training/vector-cortex/corpus/ (new generated — train/calibration/test per-head jsonl); src/config/vector-cortex-enc0c.ts (new — ENC_0C_ENABLED sprintFlag sibling, extracted so vector-cortex.ts stays at 300 soft); src/config/vector-cortex.ts (additive — re-export ENC_0C_ENABLED, exactly 300); src/config.ts (additive — re-export ENC_0C_ENABLED); src/vector-cortex/encoder/heads-candidate.ts (new delegate impl — head-candidate-v1 load/validate seam: schema + trunkDigest + dims + finiteness + sha256 gating, MEGACOMPACT_ENC_0C-gated, never force-loads); src/vector-cortex/encoder/heads.ts (additive — thin re-export of the candidate seam, survivor 249→262); src/vector-cortex/enc0c-acceptance.test.ts (new — acceptance aggregator driving ENC-HEADS-001..006, flag-agnostic with a MEGACOMPACT_ENC_0C=0 parity run); extensions/dashboard-server/routes-rag-settings-vector-cortex.ts (additive — boolDirect MEGACOMPACT_ENC_0C Settings toggle, default ON); conformance/vector-cortex/v2/encoder-heads-real/ENC-HEADS-001..006 (new fixtures); conformance/vector-cortex/v2/schemas/encoder-heads-real-fixture.schema.json (new schema); scripts/ml5-enc/gen-fixtures.mjs (extended — additive encoder-heads-real domain + owner ENC-0c, idempotent); docs/vector-cortex/evidence/ENC-0c.md (new — evidence record); docs/vector-cortex/sprints/ENC-0c-five-head-training.md (this file)`.
