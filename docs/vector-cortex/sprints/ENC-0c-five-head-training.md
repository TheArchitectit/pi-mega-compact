...
Production ownership: `training/vector-cortex/train_heads.py`; `training/vector-cortex/gen_synthetic_corpus.py`; `training/vector-cortex/train-v1.json`; `training/vector-cortex/dataset-manifest-enc0c.json`; `training/vector-cortex/corpus/*`; `src/config/vector-cortex-enc0c.ts`; `src/config/vector-cortex.ts`; `src/config.ts`; `src/vector-cortex/encoder/heads-candidate.ts`; `src/vector-cortex/encoder/heads.ts`; `src/vector-cortex/enc0c-acceptance.test.ts`; `src/vector-cortex/enc0c-candidate-acceptance.test.ts`; `extensions/dashboard-server/routes-rag-settings-vector-cortex.ts`; `conformance/vector-cortex/v2/encoder-heads-real/*`; `conformance/vector-cortex/v2/schemas/encoder-heads-real-fixture.schema.json`; `scripts/ml5-enc/gen-fixtures.mjs`; `docs/vector-cortex/evidence/ENC-0c.md`; `docs/vector-cortex/sprints/ENC-0c-five-head-training.md` (this file). Notes — train_heads.py is new trainer emitting the head-candidate-v1 tree; gen_synthetic_corpus.py writes the sibling dataset-manifest-enc0c.json never the pre-existing ML5-A manifest; corpus is synthetic zero-user-bytes 432 samples 12 groups; heads-candidate.ts is the flag-gated load+validate delegate (never force-loads), heads.ts a thin re-export survivor; ENC_0C_ENABLED defaults ON and SETTINGS-exposed, `=0` byte-identical to the ENC-0b survivor.
...
7. Add the sprint acceptance aggregator `src/vector-cortex/enc0c-acceptance.test.ts` (registration / heads-fire / flag-off byte-parity / split-boundary / determinism) plus the delegate sibling `src/vector-cortex/enc0c-candidate-acceptance.test.ts` (the head-candidate load/validate seam: load+validate, missing-head, non-finite, digest-corrupt, trunk-mismatch, flag gate) — split so both stay under the src/ 300-line soft limit. Then evidence `ENC-0c.md` recording per-head strategy, corpus digests, seed, and the flag-off byte-identical check.
