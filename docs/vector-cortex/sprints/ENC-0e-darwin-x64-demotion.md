...
Production ownership: `src/config/vector-cortex-enc0e.ts`; `src/config/vector-cortex.ts`; `src/config/vector-cortex-vc3to8.ts`; `src/config.ts`; `extensions/dashboard-server/routes-rag-settings-vector-cortex.ts`; `src/vector-cortex/encoder/runtime-select.ts`; `src/vector-cortex/encoder/runtime-emit.ts`; `src/vector-cortex/encoder/decision.ts`; `src/vector-cortex/ml5c-acceptance.test.ts`; `extensions/dashboard-server/setup-cortex-blockers.ts`; `extensions/dashboard-server/api-contracts/setup-cortex.ts`; `extensions/dashboard-server/routes-setup-cortex.ts`; `extensions/dashboard-client/src/tabs/SetupTab/CortexBlockersCard.tsx`; `extensions/dashboard-client/src/tabs/SetupTab/CortexSetup.tsx`; `extensions/dashboard-client/src/types/setup-cortex.ts`; `scripts/ml5-enc/gen-fixtures.mjs`; `src/vector-cortex/enc0e-acceptance.test.ts`; `conformance/vector-cortex/v2/encoder-demotion/*`; `conformance/vector-cortex/v2/schemas/encoder-demotion-fixture.schema.json`; `docs/vector-cortex/evidence/ENC-0e.md`; `docs/vector-cortex/sprints/ENC-0e-darwin-x64-demotion.md (this file)`. Notes: runtime-select.ts evolves the pure selection to return `demotionReason:string|null` with the injectable-platform seam (no real Mac); runtime-emit.ts additively carries `demotionReason` on `vector_cortex_runtime_selected`; decision.ts exposes the single canonical `DARWIN_X64_DEMOTION_REASON` string (plus a deterministic sentinel for the unique-failure injection); the flag sibling `vector-cortex-enc0e.ts` + the boolDirect toggle mirror the ENC-0d slice; the generator gains a fifth additive block (algorithm `encoder-demotion`, schema `schemas/encoder-demotion-fixture.schema.json`) emitting ENC-DEMO-001..006; the v2 manifest registration bump is cross-cutting.
