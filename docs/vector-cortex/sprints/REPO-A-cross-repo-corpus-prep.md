...
REPO-A is one of the 15 deferred-audit sprint docs already counted in `EXPECTED_SPRINTS = 65` in `scripts/vector-cortex-docs-check.mjs`; this sprint does NOT bump the count — the per-sprint commit leaves it at 65. Evidence doc `docs/vector-cortex/evidence/REPO-A.md` must record: synthetic-fixture status, the negative-consent test result, and whether the builder was exercised against a real donated corpus at execution time (absent donated repos, the run reports consent-absent and writes nothing — a valid completed state, not a deferral).
