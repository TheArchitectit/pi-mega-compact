...
Production ownership: `scripts/ml5/promotion-gate.mjs (evolves — accepts {color} real candidate manifests incl. the trained head weights + trunk; digest-verifies every staged byte before swap; atomic swap + rollback-to-previous on qualification failure); src/vector-cortex/encoder/promotion.ts (evolves — extend PromotionV1 to carry {color, assetDigestStack} and the atomic swap + rollback helpers; delegate-shell split where needed to stay ≤ 300); src/vector-cortex/encoder/promotion-emit.ts (new — the promote/demote/rollback event writers to events.log, append-only; non-fatal); conformance/vector-cortex/v2/encoder-promotion/ (fixtures ENC-PROMO-001..006); docs/vector-cortex/evidence/ENC-0d.md (new)`.
