...
**Status:** done | **Depends on:** VC4C | **Phase:** VC5
...
> **Ownership amendment (prod-prep 2026-08-07):** there is NO `prompt-dag/manifest.ts` — the earlier "appears in both subtrees" claim was inaccurate. The prompt-DAG digest is NOT a separate module: `dagDigest()` lives co-located in `prompt-dag/validator.ts` (~line 156) with the topological-order/rejection concern, and deliberately EXCLUDES node token counts (a planner input). The planner's distinct digest (`planManifestDigest()`/`validatePlanManifest()`) lives in `planner/manifest.ts` and covers token counts — that is what makes the post-plan token mutation detectable as `PLN_MANIFEST_DIGEST_MISMATCH`. Two concerns, two real files; no `prompt-dag/manifest.ts` was ever needed.
