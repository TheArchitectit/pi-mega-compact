...
Production ownership: `extensions/dashboard-server/routes-prefix-stability.ts (new — GET /api/prefix-stability endpoint); extensions/dashboard-server/routes-prefix-stability.test.ts (new); extensions/dashboard-server/api-contracts/prefix-stability.ts (new — response contract); extensions/dashboard-server/api-contracts/endpoints/registry.ts (additive — prefix-stability entry, EXPECTED_ENDPOINT_COUNT bump); extensions/dashboard-server/api-contracts/endpoints/registry-ext.ts (additive — the prefix-stability endpoint entry lives here rather than registry.ts, the standard VC9 additive-group pattern, keeping registry.ts under its 500-line limit); extensions/dashboard-server/api-contracts/index.ts (additive — barrel re-export for the PrefixStability types); extensions/dashboard-server/api-contracts.test/endpoints-registry.test.ts (additive — EXPECTED_ENDPOINT_COUNT 52→53 + server-path set entry); extensions/dashboard-server/route-dispatch.ts (additive — if-chain entry); extensions/dashboard-server/routes.ts (additive — barrel re-export); extensions/dashboard-client/src/tabs/CacheTab.tsx (additive — new PrefixStabilityCard section); extensions/dashboard-client/src/components/PrefixStabilityCard.tsx (new — per-turn trend chart); extensions/dashboard-client/src/api/client.ts (additive — fetchPrefixStability re-export); extensions/dashboard-client/src/api/client-http.ts (new — shared getJson/putJson/postJson/query/ApiError helpers, delegate-shell split so client.ts stays under the 400 soft limit); extensions/dashboard-client/src/api/client-extra.ts (new — fetchPrefixStability + model-thresholds trio, delegate-shell split of client.ts); extensions/dashboard-client/src/types/ (additive — PrefixStabilityResponse type); src/config/vector-cortex-pcc.ts (new); src/config/vector-cortex.ts (additive re-export, stays ≤ 300); src/config.ts (additive re-export, stays ≤ 200); extensions/dashboard-server/routes-rag-settings-vector-cortex.ts (additive boolDirect toggle, stays ≤ 300); extensions/mega-events/context-handler/tailResult.ts (FORCED DEVIATION — see below; `runtime.logger.info("prefix_stability",{...})` changed to `runtime.appendEvent("prefix_stability",{...})` so the events reach the monitoring events.log that the endpoint reads from ctx.eventsPath); scripts/pc-prompt-cache/gen-fixtures-pcc.mjs (new generator); conformance/vector-cortex/v2/prompt-cache/PC-009.json..015.json (new); conformance/vector-cortex/v2/manifest.json (additive); src/vector-cortex/pcc-acceptance.test.ts (new); scripts/vector-cortex-docs-check.mjs (EXPECTED_SPRINTS 35→36); docs/vector-cortex/sprints/PC-C-dashboard-cache-visibility.md (this spec); docs/vector-cortex/evidence/PC-C.md (new)`.

> **Forced deviation (implementation) — `extensions/mega-events/context-handler/tailResult.ts`:** the spec's source claim ("`prefix_stability` events are already logged by tailResult.ts ... reads recent `prefix_stability` events from the events log") was NOT true as written: `tailResult.ts:90` emitted via `runtime.logger.info(...)`, which writes to `mega-compact.log` (the debug-gated logger path, `{ts, level, event, ...}`), NOT the monitoring `events.log` (`{ts, event, ...}`) that `ctx.eventsPath` / the dashboard SSE consume. To make the endpoint functional as spec'd (read from `ctx.eventsPath` = `events.log`), the emission was changed from `runtime.logger.info("prefix_stability", {...})` to `runtime.appendEvent("prefix_stability", {...})` — same field shape, always-on, reaches `events.log`. This is an OUT-OF-OWNERSHIP edit to `tailResult.ts` routed through the same `config.cacheStriping`/`config.messageSeparation` gate (no behavioral change to the prompt assembly; only the emission target). It is required for the endpoint's data source to exist. Flagged for controller ratification (same pattern as PC-A/PC-B's `s29`/sector deviations).
...
- `PC-010: no events returns empty turns array` — `{ kind:"prompt-cache", flag:"MEGACOMPACT_PC_C", flag_enabled:true, events_present:false, turns_returned:0 }`. Contract resolution: the endpoint's `PrefixStabilityResponse.avgRatio` is typed `number` (task 2), so an empty window returns `avgRatio: 0` (not `null`) — the typed contract, null-free per PREVENT conventions. The fixture pins the empty-state `events_present:false` + `turns_returned:0`.
