# VC1 — Exact occurrence ledger

**Status:** planned | **Depends on:** VC0 reviewer-accepted | **Sprints:** VC1A, VC1B, VC1C

## Phase goal and boundary

Own EventV2 bytes, tool identity, conformance, compatibility journal/downgrade exporter. Inputs are only accepted predecessor evidence and the normative [contracts](../CONTRACTS.md), [evaluation](../EVALUATION.md), [triads](../TRIAD_RESILIENCE.md), and applicable asset/security/codec specifications. Outputs: **EventV2, Ledger capabilities, FixtureManifestV2**. Current storage/backend facts defer to root `CLAUDE.md` and inspected source, not stale `PLAN.md` better-sqlite text.

## Sprint boundaries

- **VC1A:** contract/schema/fixture seam and first production capability; contract review is a hard gate.
- **VC1B:** deterministic core/integration and phase-specific migration or artifact work.
- **VC1C:** conformance, chaos, qualification, powered rollout, or handoff. Each sprint may use multiple small production-first sub-sprints with full gates.
...
Handoff: VC2 receives authoritative ordered EventV2 and neutral fixtures. Update README status only after reviewer attestation.
