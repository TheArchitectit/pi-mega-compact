# VC3 — Derived cortex topology

**Status:** planned | **Depends on:** VC2 reviewer-accepted | **Sprints:** VC3A, VC3B, VC3C

## Phase goal and boundary

Own Capability-gated store, deterministic multi-head graph, structured router generation. Inputs are only accepted predecessor evidence and the normative [contracts](../CONTRACTS.md), [evaluation](../EVALUATION.md), [triads](../TRIAD_RESILIENCE.md), and applicable asset/security/codec specifications. Outputs: **CortexReader/Writer/Admin, TopologyV1, RouterKeyV2**. Current storage/backend facts defer to root `CLAUDE.md` and inspected source, not stale `PLAN.md` better-sqlite text.

## Sprint boundaries

- **VC3A:** contract/schema/fixture seam and first production capability; contract review is a hard gate.
- **VC3B:** deterministic core/integration and phase-specific migration or artifact work.
- **VC3C:** conformance, chaos, qualification, powered rollout, or handoff. Each sprint may use multiple small production-first sub-sprints with full gates.
...
Handoff: VC4 receives versioned query results and dependency/contradiction edges. Update README status only after reviewer attestation.
