# VC6 — Advanced restoration and self-healing

**Status:** planned | **Depends on:** VC5 reviewer-accepted | **Sprints:** VC6A, VC6B, VC6C

## Phase goal and boundary

Own Optimize conservative closure, exact-source restoration, derived-state rebuild controller. Inputs are only accepted predecessor evidence and the normative [contracts](../CONTRACTS.md), [evaluation](../EVALUATION.md), [triads](../TRIAD_RESILIENCE.md), and applicable asset/security/codec specifications. Outputs: **ClosureProofV2, RestoreResultV1, RepairPlanV1**. Current storage/backend facts defer to root `CLAUDE.md` and inspected source, not stale `PLAN.md` better-sqlite text.

## Sprint boundaries

- **VC6A:** contract/schema/fixture seam and first production capability; contract review is a hard gate.
- **VC6B:** deterministic core/integration and phase-specific migration or artifact work.
- **VC6C:** conformance, chaos, qualification, powered rollout, or handoff. Each sprint may use multiple small production-first sub-sprints with full gates.
...
Handoff: VC7 receives stable covered ranges/digests and dependency high-water. Update README status only after reviewer attestation.
