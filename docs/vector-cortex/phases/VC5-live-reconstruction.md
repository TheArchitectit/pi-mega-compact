# VC5 — DAG-planned validated live prompts

**Status:** planned | **Depends on:** VC4 reviewer-accepted including closure | **Sprints:** VC5A, VC5B, VC5C

## Phase goal and boundary

Own PromptDag ownership/order, budget planner, base ProviderProfile registry, renderer and powered rollout. Inputs are only accepted predecessor evidence and the normative [contracts](../CONTRACTS.md), [evaluation](../EVALUATION.md), [triads](../TRIAD_RESILIENCE.md), and applicable asset/security/codec specifications. Outputs: **PromptDagV1, PlanV1, ProviderProfileV1, RenderManifestV1**. Current storage/backend facts defer to root `CLAUDE.md` and inspected source, not stale `PLAN.md` better-sqlite text.

## Sprint boundaries

- **VC5A:** contract/schema/fixture seam and first production capability; contract review is a hard gate.
- **VC5B:** deterministic core/integration and phase-specific migration or artifact work.
- **VC5C:** conformance, chaos, qualification, powered rollout, or handoff. Each sprint may use multiple small production-first sub-sprints with full gates.
...
Handoff: VC6 receives live failures; no unclosed plan may serve. Update README status only after reviewer attestation.
