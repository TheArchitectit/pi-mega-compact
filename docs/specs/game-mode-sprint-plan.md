...
**Pre-defined TODOs:**
- [x] **S35.1** `game_achievements` DDL: `(id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL, hidden INTEGER NOT NULL DEFAULT 0, icon TEXT, unlocked_at INTEGER NULL)`. Seed the 9 achievement rows (id/title/description/hidden/icon) on first open (idempotent).
- [x] **S35.2** `game-achievements.ts` accessors — `getAchievement`, `listAchievements`, `unlockAchievement` (sets `unlocked_at` only if currently NULL, returns whether it was newly unlocked), `isUnlocked`. Parameterized (PREVENT-002).
- [x] **S35.3** `evaluateAchievements(scores)` in `src/game/scoring.ts` — pure fn over the `game_scores` aggregates: returns `{unlocked: string[], newlyUnlocked: string[]}` (newly = unlocked-but-not-yet-persisted). Conditions per §9b table (compact_count, turns max, mega_cache row, dedupe sum, repos distinct, turn level, ts hour, flawless 100%-no-overshoot, 5 compacts same session).
- [x] **S35.4** Wire evaluation into the S33 scoring hooks: after a score is recorded, call `evaluateAchievements` → for each `newlyUnlocked` id, `unlockAchievement` (records `unlocked_at`) + set a transient `achievementFlare` with the title (consumed by widget + dashboard for that turn).
- [x] **S35.5** TUI widget: when `achievementFlare` is set, render a one-line toast `🏆 Achievement unlocked: <title>` in `ansi.accent` for that turn (consumed next render). Width-safe.
- [x] **S35.6** Dashboard Game Mode tab: an achievements tile row. Hidden achievements (`hidden=1 AND unlocked_at IS NULL`) render NOTHING (no teaser) — same invariant as Opie. Unlocked achievements show icon + title + unlocked_at. Locked-but-visible achievements show `??? <title>` (teaser).
- [x] **S35.7** `GET /api/achievements` → JSON array; `metric`-style validation n/a (fixed ids). Annotated `guardrails-allow PREVENT-PI-004`.
- [x] **S35.8** `/mega-game achievements` — terse list of unlocked achievements (hidden ones only appear once unlocked).
- [x] **S35.9** Tests: achievement seed idempotency, unlock idempotency (second unlock is a no-op), `evaluateAchievements` triggers (each of the 9 conditions), hidden-achievement render gating (hidden+locked = nothing rendered), `achievementFlare` toast, parameterization (G3), `MEGACOMPACT_STATE_DIR` (G7).
- [x] **S35.10** Release notes + CHANGELOG: document `/mega-game`, the 6 themes, minimal TUI, the High Score tab, MEGA CACHE, achievements (incl. the hidden easter-egg rule). Update `INDEX_MAP.md` + `HEADER_MAP.md`.
- [x] **S35.11** Full gate green; `guardrails-scan` + `semantic-scan` clean; manual dashboard + TUI smoke test (unlock an achievement, verify toast + tile).
...
- **S35:** 11 todos (S35.1–S35.11) — ✅ DONE (commit `2b65649`). game_achievements DDL (9 seeded, idempotent ON CONFLICT DO NOTHING) + game-achievements.ts accessors (listAchievements/getAchievement/unlockAchievement/isUnlocked, parameterized, DbAchRow+toAchievementRow mapper) + evaluateAchievements(scores) pure fn (9 conditions over leaderboard aggregates) + evaluateAndUnlockAchievements orchestrator (best-effort non-fatal) + turn_end/session_compact hook wiring (achievementFlare, one-cycle TUI toast + dashboard tile pulse, mirrors megaCacheFlare) + MegaRuntime armAchievementFlare + WidgetData.achievementFlare/Titles + dashboard achievements tile row (hidden+locked renders NOTHING; visible-locked shows ???; unlocked shows icon+title+date) + transient cross-process unlock toast (detects new unlocked_at) + GET /api/achievements (405 guarded, loopback) + /mega-game achievements subcommand + 23 new tests (game-achievements, evaluateAchievements 9-condition matrix, widget flare, html tiles, server /api/achievements) + RELEASE_NOTES v0.8.0 + CHANGELOG + INDEX_MAP/HEADER_MAP. Gate green (544/544, was 514). Game-mode release complete (S30–S35).
