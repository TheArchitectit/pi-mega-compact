...
- [ ] **S32.4** `GET /api/game-state` → `{game_mode_on, theme, tui_display_mode}`. `PUT /api/game-state` → validate body, `setGameState(patch, stateDir)`, respond 200. Invalid theme → 400. **Architecture note (scout finding):** the dashboard server is a detached child process with no `MegaRuntime` ref, so it **cannot** call the in-process `bumpGameState()`. Instead, the cross-cutting **table-per-concept + fs.watch** pattern (see §7 invariant) makes the TUI widget pick up dashboard writes: `MegaRuntime` keeps an `fs.watch` on `stateDir/sqlite.db` that evicts `cachedGameState` on any write, so the next `snapshot()` re-queries. (The in-process `/mega-game` path still calls `bumpGameState()` for an instant update.) The dashboard server reads/writes the same `game_state` table the TUI reads — one source of truth.
