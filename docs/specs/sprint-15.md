...
the docs (README, RETENTION_POLICY, RUNBOOK) don't reflect v0.2.0 (sqlite,
...
- `install.sh` notes SQLite data dir.
...
2. DR      scripts/dedup-restore-drill.sh:
           - validate sqlite.db opens; context_chunks row count matches
           - recompute regionHash set; compare to storedRegionHashes
           - if sqlite.db missing/corrupt: rebuild from <sess>.checkpoints.json.gz snapshots
3. DOCS    RETENTION_POLICY.md (TTL 90d, soft-delete via dedup_status='removed', VACUUM cadence)
           ...
           README: storage=sqlite, embedder=trigram|minilm flag, flags table, MEGACOMPACT_* env
...
- [ ] `scripts/dedup-restore-drill.sh` passes: sqlite integrity OK + rebuild-from-JSON verified.
- [ ] `RETENTION_POLICY.md` + `DEDUP_RUNBOOK.md` written (< 500 lines each).
- [ ] README + CHANGELOG reflect v0.2.0 (sqlite, embedder modes, flags).
- [ ] `ci.yml` green (build + lint + test + regression). The v0.1.0 `store.test.ts`/`engine.test.ts` failures (FAIL-2026071302) were fixed and the failure resolved.
...
# v0.1.0 remains installable; sqlite data dir is forward-compatible.
```
