...
**Threshold invariant (folded 2026-08-12 — user's "something isn't following those rules" + "no hardcoded value; models exceed 1M").** The compaction fire point must always be `configuredPct × ACTUAL live ` + "`lastCtxWindow`" + ` — the window reported by the provider at runtime (context event / agent_end). **No hardcoded context-window value may appear anywhere in the firing path** — not 200k, not 1M, not any constant. Model windows differ per provider and exceed 1M, so any fixed number is wrong by construction. Verified blast radius: the ONLY hardcoded window is the `resolveThreshold()` boot fallback `Math.round(tierPct * 200_000)` (`mega-config.ts:83`); `COMPACT_TIERS` token values are label-validation only, never a firing point; all other 200k/1M matches are test fixtures. Fix: (a) the fire point's configurable % comes from the Dashboard and is **default 20% of the model's max context** (the single config knob — see "Settings surface" below); (b) when `lastCtxWindow <= 0` (window unknown), auto-compaction **DEFERS** — never substitute a guessed window; (c) the token-gate path (`pct == null`) consults the per-model Dashboard override (`resolveModelThreshold`) exactly like the percent path; (d) `custom` tier (`MEGACOMPACT_THRESHOLD_TOKENS`) is an explicit absolute, unchanged.

**Settings surface (folded 2026-08-12 — "configurable threshold in the settings, default 20% of max").** Today the compaction fire-point % (tierPct / MEGACOMPACT_TIER) is **not** a Settings toggle — it is env-only and debug-displayed (`gather-debug-keys.ts`), while the % lives in TIER_PCT (low 0.5 … mega 0.75) + DEFAULT_FIRE_POINT_PCT=70 for per-model. Requirement: the fire point is a single **dashboard Settings number** (`MEGACOMPACT_THRESHOLD_PCT`, % of the actual model window at which compaction fires) with **default 80% used** — the user frames this as "20% of max **free remaining**" (confirmed, 2026-08-12): compaction fires when free space drops to 20% of the window, i.e. at 80% used, leaving a 20% headroom reserve. Editable in the Settings tab (per the all-flags-toggleable rule — never EXCLUDED_SETTINGS); the UI shows "(leaves 20% free)" next to 80. The named-tier map (low 0.5 …) becomes preset shortcuts feeding that one %; a per-model Model Thresholds row still wins when the user sets it. **Default fire point = 0.80 used** (NOT the current tier=low 0.50). Settings UI labels the knob with the reserve framing the user anchored on ("fires at 80% used — keeps 20% free").
