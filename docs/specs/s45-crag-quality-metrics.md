...
- [ ] **S45A.3** Implement `coverage(query, hits)` metric
...
- [ ] **S45A.4** Implement `diversity(hits)` metric
...
- [ ] **S45A.5** Implement `specificity(hits)` metric
...
- [ ] **S45C.1** Add config flags to `src/config/dedup.ts` (~line 30)
...
- [ ] **S45C.4** Add monitoring event type in `src/monitoring.ts` (~line 22)
...
### Sprint S45D: Dashboard Quality Telemetry
...
- [ ] **S45D.1** Add dashboard API endpoint
...
- [ ] **S45D.2** Add quality metrics to MetricsTab
...
- [ ] **S45D.3** Verify: dashboard loads without errors when feature is OFF (404 handled gracefully)
