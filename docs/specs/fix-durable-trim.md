...
1. **Load crash** — `@mongodb-js/zstd` is `import`ed at top level of `src/store/compression.ts:35`, which the entire store layer transitively imports. Its native `zstd.node` is not in the npm tarball and is skipped under npm `allowScripts`, so a clean install fails to load the whole extension. zstd is **dead code** (only re-exported from `store.ts`; zero runtime callers outside the unit test).
