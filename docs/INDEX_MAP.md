...
| **S50 spec (per-turn metrics + fork — `raw_transcript.turn_index` wiring, `turns.epoch_id` stamping, `/mega-metrics` + `/mega-fork` commands, recall/dedup/compression rollups by turn + conversation; S50A/S50B/S50C shipped)** | [specs/s50-per-turn-metrics-fork.md](specs/s50-per-turn-metrics-fork.md) | docs/specs/ |
| **S51 spec (auto-categorizing wiki — S47 re-targeted onto the S49 store: k-means over real `context_chunks` embeddings + TF-IDF labels + extractive wiki pages, persisted in the reserved turns.db `topics`/`memory_topics` shells, every-Nth-compaction rebuild, `/mega-topics`; Wiki dashboard tab deferred to S52; S51A/S51B/S51C gated)** | [specs/s51-auto-categorizing-wiki.md](specs/s51-auto-categorizing-wiki.md) | docs/specs/ |
