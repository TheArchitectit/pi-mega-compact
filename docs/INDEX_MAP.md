...
| **S53–S55 program (prompt cache + memory — provider cache visibility S53A, memory enhancements S53B, TUI+cost S53C, prefix-stability telemetry S54, pair-atomic prompt layout S55; supersedes PROMPTCACHE_PLAN_V2 Phases 2–4)** | [specs/s53-prompt-cache-memory-program.md](specs/s53-prompt-cache-memory-program.md) | docs/specs/ |
| **remote-branch gap analysis 2026-07-29 (5 origin branches inventoried; promptcache docs verified; empty memory branch; worktree-bench merge guidance; empty Cache tab root-cause)** | [BRANCH_GAP_ANALYSIS.md](BRANCH_GAP_ANALYSIS.md) | docs/ |
