...
| **S50 spec (per-turn metrics + fork — `raw_transcript.turn_index` wiring, `turns.epoch_id` stamping, `/mega-fork` + `/mega-turns` commands, cache-hit/dedup/compression rollups by turn + conversation; S50A/S50B/S50C gated)** | [specs/s50-per-turn-metrics-fork.md](specs/s50-per-turn-metrics-fork.md) | docs/specs/ |
