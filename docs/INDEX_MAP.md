...
| **LTS fix: threshold percent normalization + output-error catch — umbrella-OFF also DEFERS on unknown window (no guessed 200k); COMPACT_TIERS fold TIER_PCT; `autoCompactCheck` required; `savedGoal` from effectiveThreshold; named env defaults; truncated-response catch (Phases A–H)** | [specs/threshold-percent-normalization.md](specs/threshold-percent-normalization.md) | docs/specs/ |
