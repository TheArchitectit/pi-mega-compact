...
// ─── VC7C cache-diagnostics fixture schema (CACHE-016..030 + named) ────────────
// Inlined shapes (no $ref): the conformance validator implements only a schema
// subset, so a $ref node type-checks as nothing and validates silently.

/** A payload-FREE miss observation: digests and counts only, NEVER request bytes. */
function diagObservationSchema() {
  return {
    type: "object",
    required: [
      "requestProfileId",
    ...
    properties: {
      ...
      requestProfileVersion: { type: "string" },
      cachedProfileId: { type: ["string", "null"] },
      cachedProfileVersion: { type: ["string", "null"] },
      requestCoveredDigest: { type: "string", pattern: "^sha256:[0-9a-f]+$" },
      cachedCoveredDigest: { type: ["string", "null"], pattern: "^sha256:[0-9a-f]+$|^null$" },
      requestedRangeCount: { type: "integer", minimum: 0 },
      cachedRangeCount: { type: "integer", minimum: 0 },
      requestDigest: { type: "string", pattern: "^[0-9a-f]+$" },
      cachedRequestDigest: { type: ["string", "null"], pattern: "^[0-9a-f]+$|^null$" },
      requestDependencyHighWater: { type: "integer", minimum: 0 },
      cachedDependencyHighWater: { type: ["integer", "null"], minimum: 0 },
...
schemas["schemas/cache-diagnostic-fixture.schema.json"] = {
  $schema: "https://json-schema.org/draft-07/schema#",
  title: "VC7C cache-diagnostics fixture envelope",
  description:
    "Common structure every VC7C cache-diagnostics fixture validates against. CACHE-016..030 + named CACHE-MISS-001 / CACHE-STALE-003 pin the EXCLUSIVE miss classification order (profile -> range -> dependency -> request -> generation -> unknown) produced by the PURE classifier classifyMiss(observation). PAYLOAD-FREE BY CONSTRUCTION: input.observe carries only digests (sha256-prefixed covered digest; BARE lowercase hex request digest), profile id/version, bounded range counts, a dependency high-water integer, and a generation-invalidated boolean — never a request payload, never a session id, never covered ranges of user content. The classifier is pure and deterministic; a given observation yields exactly ONE class every time. `expected.missClass` is the single deterministic class the acceptance test asserts classifyMiss returns; `expected.evidence` is the payload-free MissEvidence (booleans + bounded counts) the classifier must produce. CACHE-MISS-001 asserts that a profile digest mismatch classifies PROFILE ONLY (even when range/dependency also differ, profile wins by the ranking), and CACHE-STALE-003 asserts that an invalidated M6 generation classifies GENERATION (a crystal from a dead generation cannot be served). All rows are fed verbatim into the REAL production module src/vector-cortex/cache/diagnostics.js, no mocks. Every digest is a real SHA-256 over that field's own notional content, computed by the generator, never hand-written.",
  type: "object",
  required: ["id", "producer", "assertion", "kind", "expected", "input"],
  properties: {
    id: { type: "string" },
    ...
    expected: {
      ...
      properties: {
        ...
        evidence: {
          ...
          properties: {
            profileMismatch: { type: "boolean" },
            rangeMismatch: { type: "boolean" },
            dependencyAdvanced: { type: "boolean" },
            requestMismatch: { type: "boolean" },
            generationInvalidated: { type: "boolean" },
            requestedRangeCount: { type: "integer", minimum: 0 },
            cachedRangeCount: { type: "integer", minimum: 0 },
            dependencyDelta: { type: "integer", minimum: 0 },
            absent: { type: "boolean" },
          },
        },
      },
    ...
    input: {
      ...
      required: ["scenario"],
      properties: {
        scenario: { type: "string", enum: ["classify"] },
        observe: diagObservationSchema(),
      },
    },
  },
};

// ─── VC7C M5 request-hash-v2 migration fixture schema (M5-001..020 + named) ─────

/** A single predecessor (v1) request-hash row: profile + BARE hex request digest. */
function m5V1RowSchema() {
  return {
    type: "object",
...
schemas["schemas/request-hash-v2-fixture.schema.json"] = {
  $schema: "https://json-schema.org/draft-07/schema#",
  title: "VC7C M5 request-hash-v2 migration fixture envelope",
  description:
    "Common structure every VC7C M5 request-hash-v2 fixture validates against. M5-001..020 + named M5-COLLIDE-002 pin the COMPLETED copy/validate/SWITCH migration driven by migrateRequestHashV2(host): v2 folds each profile's economics version into the cache-identity hash so a pricing/TTL/exclusion change cannot reuse a v1-minted identity; the migration is IDENTITY-PRESERVING (every v2 row carries the SAME requestDigest as its v1 source); and the switch requires ZERO collisions (no two distinct v1 rows map to one v2 hash) or it refuses with M5_REQUEST_HASH_COLLISION. M5-COLLIDE-002 asserts that TWO DISTINCT v1 rows mapping to ONE v2 hash BLOCK the switch. input.v1Rows is the predecessor set; input.econVersionOf maps each profileId to the economics version folded in; input.activeVersion is the pointer before switching (must be 1). expected.codes is the exact ordered failure-code list (empty = ok) the acceptance test asserts migrateRequestHashV2 returns; a collision injected AFTER validation must be caught at switch time because the switch re-validates against freshly-read host state. All rows are fed verbatim into the REAL production module src/vector-cortex/migrations/request-hash-v2.js, no mocks. Digests are real SHA-256 values computed by the generator from each row's own fields, never hand-written.",
  type: "object",
  required: ["id", "producer", "assertion", "kind", "expected", "input"],
  properties: {
    id: { type: "string" },
    ...
    input: {
      type: "object",
      required: ["v1Rows", "econVersionOf", "activeVersion"],
      properties: {
        scenario: { type: "string", enum: ["migrate"] },
        v1Rows: { type: "array", items: m5V1RowSchema() },
        econVersionOf: {
          type: "object",
          additionalProperties: { type: "string" },
        },
        activeVersion: { type: "integer", enum: [1, 2] },
        sessionOf: {
          type: "object",
          additionalProperties: { type: "string" },
        },
