...
export function createTurnStore(stateDir: string = getStateDir()): TurnStore {
  ...
  function listConversationTurns(conversationId: string): TurnRow[] {
    const rows = db
      .prepare("SELECT * FROM turns WHERE conversation_id = ? ORDER BY turn_index ASC")
  ...
  function ensureConversationId(sessionId: string): string {
    ...
    const key = `conv_${sid}`;
    const existing = db.prepare("SELECT value FROM turns_meta WHERE key = ?").get(key) as
      | { value: string }
      | undefined;
    if (existing) return existing.value;
    const conv = newConversationId();
    db.prepare("INSERT INTO turns_meta (key, value) VALUES (?, ?)").run(key, conv);
    return conv;
  }

  function forkConversation(parentConversationId: string, forkTurnId: number): ForkResult {
  ...
  function pruneTurns(opts: PruneOptions): { deletedTurns: number; deletedRecall: number } {
    ...
    withTx(db, () => {
      ...
      const r1 = db.prepare(`DELETE FROM turn_recall WHERE turn_id IN (${ph})`).run(...ids);
      const r2 = db.prepare(`DELETE FROM turns WHERE id IN (${ph})`).run(...ids);
