...
test("forkConversation lineage + replay set", () => {
	...
	const turnId = store.recordTurn({
		conversationId: parent,
		sessionId: "sess_parent",
		turnIndex: 3,
	});
	store.recordTurnRecall(turnId, [
		{ checkpointId: "cp_fork", score: 0.77, source: "flat" },
	]);
	const { conversationId: child, recalled } = store.forkConversation(
		parent,
		turnId,
	);
...
test("clearTurns cascades recall rows", () => {
	...
	store.recordTurnRecall(id, [
		{ checkpointId: "cp", score: 0.5, source: "flat" },
	]);
...
test("pruneTurns keeps min-per-conversation + preserves fork points", () => {
	...
	const { deletedTurns } = store.pruneTurns({
...
test("ISOLATION: turn writes go to turns.db, not the memory sqlite.db", () => {
	...
	assert.ok(
		!files.includes("sqlite.db"),
		"turn writes must not create/open the memory sqlite.db",
	);
});
...
test("REUSE-CLEAN: no pi imports anywhere in src/store/turns/", () => {
	...
	for (const f of readdirSync(here)) {
		...
		assert.ok(
			!banned.test(src),
			`${f} must not import pi runtime / extensions (reuse contract)`,
		);
	}
});
