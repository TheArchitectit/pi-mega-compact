...
export function minhashSignature(text: string): number[] {
  ...
  for (let i = 0; i < NUM_HASHES; i++) {
    ...
    for (const x of grams) {
      ...
      // double-precision safe-integer range (2^53). A naive `(a*(x%P))%P` loses
      // precision and yields a deterministic-but-mathematically-wrong signature
      // (verified: a=x=p-1 → lossy 2147483644 vs exact 1), which degrades LSH
      // bucketing away from the theoretical Jaccard curve. Compute in BigInt then
      // coerce back — correct by construction, and cheap (~5ms for a full 256-hash
      // × 500-shingle signature; minhash runs only on compaction).
