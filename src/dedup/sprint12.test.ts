...
import { setDedupStatus } from "../store/sqlite/checkpoints.js";
...
// upsertCheckpoint'd them to active, defeating SemDeDup. add() now filters
// removed rows like search() already does. We mark one row removed directly
// (setDedupStatus) and seed a DISSIMILAR survivor, so the re-added content
// matches ONLY the removed row — isolating the resurrection behavior.
test("add() does NOT resurrect a SemDeDup-removed checkpoint (H1)", () => {
  ...
  assert.equal(
    vectorList(s, "sess_h1").find((c) => c.checkpointId === "chkpt_001")?.dedupStatus,
    "removed",
  );
  ...
  // Post-fix: removed rows are filtered from `all`, no match → new row.
  const r = s.add({
    sessionId: "sess_h1",
    summary: removedText,
  ...
  const activeHits = vectorSearch(s, "sess_h1", removedText, 10);
  assert.ok(
