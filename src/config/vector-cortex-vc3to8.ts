/**
 * config/vector-cortex-vc3to8.ts — VC3A..VC8B flag siblings (extracted).
 *
 * Mechanical extract from vector-cortex.ts so that barrel stays under the
 * 300-line soft-as-hard cap (each sprint adds one re-export line; the VC3A..VC8B
 * inline definitions were the bulk). Mirrors the vector-cortex-early.ts precedent
 * (VC0A..VC2C extracted there) and the VC8C+/ML5/ENC sibling-extract pattern.
 * vector-cortex.ts re-exports every flag below, so no consumer import path changes.
 *
 * Pi-agnostic, dependency-free (PREVENT-PI-004 / PREVENT-011). Default ON;
 * `=0` byte-identical to the predecessor for every flag.
 */
