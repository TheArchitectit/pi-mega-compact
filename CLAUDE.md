...
* **Key source files**: `src/store.ts` (state dir + JSON DR helpers + compression re-exports), `src/store/compression.ts` (versioned compressSmart/decompressSmart + async zstd helper), `src/store/sqlite.ts` (SQLite context_chunks + session_state + raw_transcript + checkpoint_epochs + dedup_mirror, FTS5 trigram), `src/store/migrate.ts` (JSON→SQLite migration), `src/store/turns/types.ts` (S49A TurnStore contract — TurnReader/Writer/Admin, capability-gated), `src/store/turns/sqlite-store.ts` (SqliteTurnStore — turns.db, fork seed recall, position-stable checkpoint), `src/store/turns/memory-store.ts` (InMemoryTurnStore — Map-backed, same contract), `src/store/turns/connection.ts` (turns.db connection manager), `src/store/turns/index.ts` (barrel + createTurnStore factory), `src/vectorStore.ts` (VectorStore add/search/dedupe), `src/engine.ts` (compactSession), `src/recall.ts` (recallAndInline), `src/embedder.ts`, `src/compact.ts`, `src/extractive.ts`, `src/supersede.ts`, `src/boundary.ts`, `src/types.ts`, `src/config.ts`, `src/adapt.ts`, `src/log.ts`.
