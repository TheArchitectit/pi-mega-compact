...
- **fix(error-retry): bare 0-token errors corroborate via the repeat detector (R9).** 0-token ≠ deterministic rejection when a router fronts the provider (the request never reaches any model). Signal 3 now returns `transient`; a narrow `bare-0-token-error` signature fallback lets the repeat detector upgrade at threshold (default 3). Text-specific deterministic signals stay first-turn poisoned — the 2026-07-28 behavior is preserved.
- **feat(error-retry): provider-outage advisory (R10).** When `consecutiveErrors` crosses `providerOutageAdviseThreshold` (default 3, `MEGACOMPACT_PROVIDER_OUTAGE_THRESHOLD`, 0 disables), ONE calm user-role advisory per outage episode: provider is flapping, retries continue automatically, context is fine — do NOT clear or reset. Resets on success; distinct from the poisoned `/clear` advise. Dashboard event `provider_outage_advised`.
- **feat(error-retry): decision observability (R11).** `classifyErrorDetailed()` returns `{ category, signal, httpStatus }` naming the exact rule that fired; poisoned/outage dashboard events + logs now carry `signal`, `rawText` (500-char cap), and `httpStatus`. New `repeat-upgrade-declined` log makes the guard visible.
- **feat(error-retry): signature normalization (R12).** `extractErrorSignature` normalizes volatile tokens (model aliases, IPs, hex ids, retry counts, 4+-digit numbers) so alternating-but-equivalent errors accumulate repeat count instead of resetting it. 3-digit HTTP statuses survive; the R9 fallback is preserved.
- **Tests:** 19 new red-first regression tests in `mega-compact-s38.test.ts` (77/77). Full suite: 0 failures across 89 files; lint + guardrails + regression clean. Details: `docs/AI_ERROR_RETRY_FINDINGS.md` §8.

---

## v0.11.3 (2026-07-30) — Known-Retryable Guard for Poisoned-Context Upgrade

- **fix(error-retry): R3 transient→poisoned-context upgrade now skips known-retryable errors.** The repeat-detection logic upgraded `transient` → `poisoned-context` after `poisonedContextRepeatThreshold` (default 3) identical error signatures regardless of error type — an API timeout ×3 fired the `/clear or /new` poisoned-context message, a false positive. Fix: a single source of truth, `KNOWN_RETRYABLE_TRANSIENT_PATTERN` / `isKnownRetryableTransient()` in `error-classifier.ts`, now defines which errors are known-retryable (network, timeout, 5xx, 429/rate-limit, max-output-token) and is shared by BOTH the classifier's transient-marker stage and the repeat-upgrade guard in `agent-handlers.ts`, so the two can no longer drift. Coverage includes the incident's actual "timed out" phrasing (which `/timeout/` does not match), `ETIMEDOUT`, `socket hang up`, and 429/rate-limit — all of which slipped through the first-pass regex. Also fixed deeper down: a 0-token "timed out" turn was classified poisoned on the FIRST turn via the 0-token signal (markers are now checked before it). Non-network deterministic rejections (like the 2026-07-28 "Request failed — please retry." incident) still upgrade correctly. Regression tests: R7(a)–R7(g) + classifier units in `mega-compact-s38.test.ts` (written red-first). Docs updated in `AI_ERROR_RETRY_FINDINGS.md §7`.
...
- **feat(s39): sessions tab with stacked memory graph + SSE real-time updates (`e73eba4` → `62814a1`).** New SQLite tables `session_heartbeats` + `token_samples` with a heartbeat hook that records token usage on every `turn_end`. Backend endpoints: `GET /api/sessions` (snapshot) + `GET /api/sessions/timeseries` (per-session time-series, `?since=` + `?sessionId=`). SSE stream at `/api/sessions/events` using `Last-Event-ID` resumable-stream pattern. Frontend: `recharts` `AreaChart` with stacked fills, auto-updating every heartbeat. Files: `extensions/mega-events/session-heartbeat.ts` (new), `extensions/dashboard-server/routes.ts`, `extensions/dashboard-client/src/SessionsTab.tsx` (new).
- **feat(s40): per-session context gauges on Overview tab (`3ae34b6` → `bb460cc`).** Overview tab shows a stacked bar of per-repo context usage real-time via `/api/sessions` + SSE. Each session gets a proportional gauge showing context tokens vs compact limit. `lastCtxTokens` fallback handles providers that don't report usage metadata. Files: `extensions/dashboard-client/src/OverviewTab.tsx` (+40), `extensions/mega-events.ts` (wire up).
- **fix(s40): `lastCtxTokens` fallback when provider reports no usage (`90a7b78`).** Providers without `usage.total_tokens` / `usage.input_tokens` metadata gracefully fall back to the last known value instead of showing 0.
- **chore(deps): install + pin `recharts` in dashboard-client (`a2319a1`, `fb2f2bf`).** S39's `SessionsMemoryChart.tsx` imported `recharts`, which was declared in `package.json` but never installed — causing the deploy.sh vite build step to fail. Reinstalled; pinned to `^2.15.4`.
- **feat: Buy Me a Coffee support link (`9e6316f`).** Adds a "Buy Me a Coffee" button to the dashboard.
- **style: biome reflow on OverviewTab + SessionContextGauges (`87948ec`).**
- **chore(release): v0.8.21 (`e81a1c4`).** Version bump 0.8.20 → 0.8.21 + rebuilt dashboard dist.

No migration required — fully backward-compatible. Full gate: 628 pass / 0 fail in ~29s. npm-only distribution; published via `./scripts/deploy.sh 0.8.21`.

## v0.8.16 (2026-07-24) — TUI Footer-Flip Fix + Cross-Process Widget Refresh + Re-Entrant Snapshot Hang Fix

- **fix(state): replace re-entrant `snapshot()` in `fs.watch` with lightweight `refreshWidgetGameState` (`cc80a14`).** The `fs.watch` game-state callback called the full `this.snapshot(ctx)`, which recomputes 6 sync SQLite opens + `writeFileSync(dashboard.json)` + store writes; those store writes retrigger the SAME `fs.watch` callback (fires on every `sqlite.db*` write, including context-event checkpoint writes) → re-entrant thrash → 190s test timeout under `mega-compact.test.js` / `mega-teamrun.test.js`. New `refreshWidgetGameState(ctx)` re-reads ONLY the `game_state` row (cache already evicted in the callback) and patches the three game-mode fields (`gameMode`/`theme`/`tuiMode`) on the existing `widgetData`, then re-registers the factory via `renderWidget()`; writes nothing to the store or `dashboard.json`, so it cannot retrigger itself. **P2 cross-process refresh:** a dashboard-made theme/toggle/tui-mode change now reflects in the live TUI immediately even when pi is idle (no context event to drive `snapshot()`); `lastWidgetCtx` stashed on every `snapshot()` so the `fs.watch` callback can force a widget re-render. Guard: no-op when `widgetData` is null or `ctx` undefined. `extensions/mega-runtime/state.ts` (+56).
- **fix(tui): P1 footer-flip + P3 dashboard parity (`34f0885`).** **P1:** the L5 ticker branch re-picked the head text every 250ms via `step = floor(Date.now()/250)`, flipping the footer line on a 250·N ms cycle (client-reported "last line keeps flipping"); now pins to the most-recent entry so the last line holds stable until a real state change. **P1 dedupe:** `pushTicker` skips consecutive identical entries (keeps ring at `TICKER_MAX` for real variety; `at` NOT refreshed on a skip). **P3:** "Next theme →" cycle button added to the dashboard ConfigTab (React) + the legacy `html.ts` settings strip — client-side cycling over already-fetched `<option>` values, no new endpoint, stays in sync with the server's `THEMES` list; `tui_display_mode` hint span `(affects the in-app TUI widget; not shown here)` added next to the TUI select. Tests: footer-stability (byte-identical across 500ms) + `pushTicker` dedupe (consecutive identical skipped, capped at `TICKER_MAX`). Files: `extensions/dashboard-server/html.ts`, `extensions/mega-runtime/state.ts` (+8 `pushTicker` dedupe), `extensions/mega-runtime/state.test.ts` (+33), `extensions/mega-runtime/widget.ts` (+8 footer pin), `extensions/mega-runtime/widget.test.ts` (+48), `extensions/dashboard-client/src/ConfigTab.tsx`.
- **chore(deploy): track `scripts/deploy.sh` — the authoritative publish gate (`3566f3d`).** Recovers the MANDATORY release pipeline script (CLAUDE.md: "never publish by hand"). Enforces clean tree + full gate + dashboard-bundle verify before `npm publish`. Was untracked in a stash; now committed so every release runs it.
- **chore(release): v0.8.16 (`fd65657`).** Version bump 0.8.15 → 0.8.16 + rebuilt dashboard dist.
- **chore(release): sync `package-lock.json` to v0.8.16 (`9e34975`).** `deploy.sh` stages only `package.json` + dist (not the lockfile); this follow-up syncs the lockfile version field so the tree is clean for the next release run.

No migration required — bug fixes only. Full gate: 609 pass / 0 fail in ~30s. npm-only distribution; published via `./scripts/deploy.sh 0.8.16`.
...
- **feat(game): S35 achievements capstone.** `game_achievements` table (9 seeded, idempotent) + `game-achievements.ts` accessors (`listAchievements`/`getAchievement`/`unlockAchievement`/`isUnlocked`, parameterized) + `evaluateAchievements(scores)` pure fn over leaderboard aggregates + `evaluateAndUnlockAchievements` orchestrator + hook wiring (turn_end + session_compact) + transient `achievementFlare` (one-cycle TUI toast + dashboard tile pulse) + dashboard achievements tile row (hidden+locked renders nothing; visible-locked shows `???`; unlocked shows icon+title+date) + transient unlock toast + `GET /api/achievements` (405 guarded, loopback) + `/mega-compact-settings achievements` subcommand (alias `/mega-game achievements`). 9 achievements: first_compact, compact_streak, turn_veteran, level_five, dedupe_master, repo_explorer, night_owl, flawless, opie_wild_ride (hidden).
- **refactor(game): command rename.** `/mega-game` → `/mega-compact-settings` as the primary command name; `/mega-game` retained as a backward-compat alias (one shared handler).
