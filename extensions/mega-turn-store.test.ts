...
test("flag ON → writes land in turns.db (not main sqlite.db)", () => {
  ...
  const turnId = recordTurnWrite(c, { conversationId: conv, sessionId: "sess_on", turnIndex: 0 }, dir);
...
test("flag OFF → writes land in main sqlite.db (legacy S48 path)", () => {
  ...
  const turnId = recordTurnWrite(c, { conversationId: conv, sessionId: "sess_off", turnIndex: 0 }, dir);
  ...
  const rows = main.prepare("SELECT * FROM turns WHERE session_id = ?").all("sess_off") as unknown[];
