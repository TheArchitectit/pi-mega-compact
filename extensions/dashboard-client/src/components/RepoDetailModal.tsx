...
export function RepoDetailModal({ repo, onClose }: RepoDetailModalProps): React.ReactElement {
  // Close on Escape key.
  useEffect(() => {
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-grid">
          <div className="modal-stat">
            ...
            <span className="modal-value">{repo.checkpointCount.toLocaleString()}</span>
          </div>
          <div className="modal-stat">
            <span className="modal-label">Sessions</span>
            <span className="modal-value">{repo.sessions.toLocaleString()}</span>
          </div>
        ...
        <dl className="modal-dl">
          ...
          <dt>Reasoning</dt>
          <dd>{repo.reasoning === null ? "—" : repo.reasoning ? "on" : "off"}</dd>
        </dl>
        <h3>Activity</h3>
        <dl className="modal-dl">
