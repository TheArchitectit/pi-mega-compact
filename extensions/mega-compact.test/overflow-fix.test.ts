...
test("FIX 1: criticalOver arms on token-pressure when pct is null (OpenAI-compat provider)", async () => {
  ...
  assert.ok(res, "live-trim returned a trimmed view (not undefined/raw overflow)");
...
test("FIX 1+2 scale per-model: a 1M-window model keeps more tail than a 200K model", async () => {
  ...
  const res200k = await h.fire("context", { type: "context", messages: h.session }, ctx200k);
  ...
  const res1m = await h2.fire("context", { type: "context", messages: h2.session }, ctx1m);
    ...
    v.messages.reduce(
      (n: number, m: any) =>
        ...
        (typeof m.content === "string"
          ? m.content.length
          : Array.isArray(m.content)
            ? m.content.reduce((s: number, c: any) => s + (c.text?.length ?? 0), 0)
            : 0),
      0,
    );
  assert.ok(
